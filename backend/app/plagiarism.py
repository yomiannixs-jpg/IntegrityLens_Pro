...
def _fingerprints(text, n=EXACT_NGRAM):
    toks=words(text)
    grams=ngrams(toks,n)
    return {hashlib.blake2b(" ".join(g).encode("utf-8"),digest_size=8).digest() for g in grams}
...
def compare_document(text,sources):
    ...
    # Phase 1: deterministic exact fingerprint screening. This validates literal reuse
    # independently of TF-IDF thresholds and gives the fuzzy stage a strong prior.
    ...
    for src in sources:
        ...
        for w in _source_windows(src.get("text") or ""):
            if len(words(w))>=6:
                windows.append(w); owners.append(src); source_fp.append(_fingerprints(w))
    ...
    if eligible and windows:
        for m in eligible:
            fp=doc_fp[m["index"]]
            if not fp: continue
            for wi,sfp in enumerate(source_fp):
                overlap=len(fp & sfp)
                if overlap:
                    ratio=overlap/max(1,len(fp))
                    if overlap>=2 or ratio>=0.45:
                        fingerprint_hits+=1
                        src=owners[wi]; sc=min(1.0,0.72+0.06*overlap+0.15*ratio)
                        if sc>best[m["index"]]["score"]:
                            best[m["index"]]={"score":sc,"classification":"exact_or_near_exact","source_id":src.get("id"),
                              "source_title":src.get("title"),"source":src.get("source"),"matched_text":windows[wi],
                              "quoted":is_quoted_sentence(m["text"]),"discovery_score":src.get("discovery_score",0),
                              "cosine":0,"sequence":0,"g4":0,"g5":0,"g6":ratio,"fingerprint_overlap":overlap}

    # Phase 2: one global sparse retrieval matrix, then expensive checks only on top candidates.
    if eligible and windows:
    ...
    return {"overall_similarity":round(max(0,mc-qc),1),"matched_word_coverage":round(mc,1),
      ...
      "engine":{"document_sentences":len(meta),"retrieval_sentences":len(eligible),"source_windows":len(windows),
                "fingerprint_ngram":EXACT_NGRAM,"fingerprint_hits":fingerprint_hits,
