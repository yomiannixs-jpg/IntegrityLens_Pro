...
app=FastAPI(title="IntegrityLens Pro v2.4 API",version="2.4.0")
...
@app.get("/")
def root():return {"ok":True,"message":"IntegrityLens Pro v2.4 API is running","version":"2.4.0"}
@app.get("/health")
def health():return {"ok":True,"version":"2.4.0"}
...
def analyze_text(text,include_web,exclude_references=True):
    ...
    private=[{"id":d.id,"title":d.title,"source":d.source,"text":d.text,"query_hits":0,
              "stages":["private"],"fetch_kind":"private","discovery_score":0} for d in list_documents()]

    web={"enabled":False,"sources":[],"queries":[],"diagnostics":{"api_key_configured":False,"api_connected":False,
         "queries_attempted":0,"queries_successful":0,"query_failures":0,"candidate_urls":0,
         "pages_fetched":0,"pdfs_fetched":0,"pages_failed":0,
         "stages_used":{"exact12":0,"exact8":0,"keywords":0},"last_error":None},
         "message":"Web discovery was not requested."}

    if include_web:web=discover_web_sources(analysis_text)

    plagiarism=compare_document(analysis_text,private+web["sources"])
    ai=analyze_ai_likelihood(analysis_text)
    ...
    return {
        ...
        "summary":{
            ...
            "analyzed_word_count":len(words(analysis_text)),
            "excluded_reference_words":len(words(refs)),
        ...
        "web_discovery":{
            "enabled":web["enabled"],"message":web["message"],"source_count":len(web["sources"]),
            "diagnostics":diag,"queries":web["queries"][:40]
        },
        "plagiarism":plagiarism,
        "ai_detection":ai,
        "limitations":[
            "Similarity is evidence of textual overlap, not automatically misconduct.",
            "Reference detection, quotation detection, and paraphrase classification are heuristic.",
            "Blocked or inaccessible pages may reduce source coverage.",
...
def self_test():
    source=("IntegrityLens validation passage demonstrates deterministic exact matching across a known source. "
    ...
    submitted=("This introduction is original filler for the validation exercise. "+source+
               " Additional original filler follows the copied benchmark passage and should remain unmatched.")
    src={"id":"selftest","title":"IntegrityLens built-in validation source","source":"internal://self-test",
         "text":source,"query_hits":1,"stages":["self-test"],"fetch_kind":"private","discovery_score":100}
    result=compare_document(submitted,[src])
    passed=result["exact_near_exact_coverage"]>0 and len(result["sources"])>0
    return {"ok":passed,"version":"2.4.0","expected":"non-zero exact match with attributed source",
            "result":result}
